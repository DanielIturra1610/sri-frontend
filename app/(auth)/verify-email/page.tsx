...
function VerifyEmailContent() {
  ...
  const handleVerifyEmailDirect = async () => {
    ...
    setMessage('');
    try {
      const response = await AuthService.verifyEmailDirect(email);
      setStatus('success');
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8">
        <div className="text-center">
          ...
          <div className="space-y-3">
            {email && (
              <button
                onClick={handleVerifyEmailDirect}
                disabled={isVerifying}
                className="block w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-medium py-2.5 px-4 rounded-lg transition-colors text-center"
              >
                Intentar nuevamente
              </button>
            )}
            <Link
              href="/register"
              className="block w-full bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-900 dark:text-white font-medium py-2.5 px-4 rounded-lg transition-colors text-center"
            >
        ...
        {email && (
          <div className="space-y-4">
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Haz clic en el botón para verificar tu correo electrónico y continuar.
            </p>
            <button
              onClick={handleVerifyEmailDirect}
              disabled={isVerifying}
